...
def train(model, device, train_loader, optimizer):
    ...
    for batch_idx, data in enumerate(train_loader):
        control_point, control_changed_point, cloud, label = [i.to(device) for i in data]
        optimizer.zero_grad()
        output = model(control_point, control_changed_point, cloud)
        loss = F.l1_loss(output, label)
        loss.backward()
        optimizer.step()
        train_loss.append(loss.item())
        train_rmse.append(torch.sqrt(F.mse_loss(output, label)).detach().cpu().numpy())
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data in test_loader:
            control_point, control_changed_point, cloud, label = [i.to(device) for i in data]
            output = model(control_point, control_changed_point, cloud)
            test_loss.append(F.mse_loss(output, label, reduction='mean').item())  # sum up batch loss
            test_rmse.append(torch.sqrt(F.mse_loss(output, label)).detach().cpu().numpy())
...
def init_dataset(base_dir, dataset_folder, train_percent, train_kwargs, test_kwargs):
    ...
    train_loader = DataLoader(TensorDataset(control_dataset[:n_train], control_changed_dataset[:n_train],
                                            cloud_dataset[:n_train], cloud_changed_dataset[:n_train]), **train_kwargs)
    test_loader = DataLoader(TensorDataset(control_dataset[n_train:], control_changed_dataset[n_train:],
                                           cloud_dataset[n_train:], cloud_changed_dataset[n_train:]), **test_kwargs)
...
def run(Module, dataset_folder, base_dir='data/generate_dataset/',
        train_percent=0.8, batch_size=8, epochs=400, lr=1e-3, gpu_idx=None):
